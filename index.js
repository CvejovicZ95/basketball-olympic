...
groups.forEach((group) => {
  ...
  standings.forEach((team, index) => {
    ...
    console.log(
      `${index + 1}. ${team.name.padEnd(12)} ${team.wins} / ${team.losses} / ${team.points} / ${team.pointsFor} / ${team.pointsAgainst} / ${pointDifferenceSign}${team.pointDifference}`,
    );
  });
  console.log("\n");
});
...
rankedTeams.forEach((team) => {
  console.log(
    `${team.rank}: ${team.name} (${team.points} / ${team.pointsFor} / ${team.pointsAgainst} / ${team.pointDifference})`,
  );
});
...
quarterfinals.getResults().quarterfinals.forEach((match) => {
  console.log(
    `${match.matchNumber}: ${match.teamA} vs ${match.teamB} - Rezultat: ${match.result} | Pobednik: ${match.winner}`,
  );
});
console.log("----------");
...
console.log("Finale:");
console.log(
  `1. ${finalResult.teamA} vs ${finalResult.teamB} - Rezultat: ${finalResult.result} | Pobednik: ${finalResult.winner}`,
);
console.log("----------");
...
console.log(
  `1. ${thirdPlaceResult.teamA} vs ${thirdPlaceResult.teamB} - Rezultat: ${thirdPlaceResult.result} | Pobednik: ${thirdPlaceResult.winner}`,
);
console.log("----------");
...
const secondPlace =
  finalResult.winner === finalResult.teamA
    ? finalResult.teamB
    : finalResult.teamA;
